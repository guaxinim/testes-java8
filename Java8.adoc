...
Como retornar de um Stream novamente para uma lista?

Para isso preciso usar a Supllier que recebe 3 argumentos (3 interfaces funcionais):
1 - Uma Factory que vai criar o objeto que sera devolvido no final da coleta
2 - O metodo que sera invocado para adicionar cada elemento
3 - Pode ser evocado numa estrategia de adicionar mais de um elemento ao mesmo tempo (paralelo)
...
A interface Collector é:
Alguém que tem um Supplier, um Acumulator e um Combiner

MAS já existem métodos prontos que retornam collectors para lista:
----
usuarios.stream()
        .filter(u -> u.getPontos() > 100)
